...
class Solution:
    ...
    Time: O(2^N) N is the number of digits in input. This is logarithmic wrt to brute force so it is good.
    ...
    def get_jumping_numbers(self, n):
        ...
        jumping_numbers_greater_than_10 = self.helper_get_jumping_numbers(n)
        
        return jumping_numbers_less_than_10 + jumping_numbers_greater_than_10

    def helper_get_jumping_numbers(self, max_number):
        number_of_digits = len(str(max_number))
        
        for depth in range(2, number_of_digits+1):
            for number in range(1, 10):
                self.generate_jumping_numbers_for_n_digits(number, 1, depth, max_number, number)
